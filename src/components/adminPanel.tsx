...
import { Trash2, Edit2, PlusCircle, Save, X } from 'lucide-react';
...
export default function AdminPanel({ games, addGame, updateGame, removeGame }: AdminPanelProps) {
  ...
  const handleSave = () => {
    ...
    if (editingGame.id) {
      ...
      // Cria um ID temporário se estiver adicionando localmente (remova se o backend criar o ID)
      const newGame = { ...editingGame, id: Date.now().toString() };
      addGame(newGame as Game);
    }
    setEditingGame(null); // Fecha o formulário
  };

  // Função auxiliar para lidar com inputs numéricos e arrays
